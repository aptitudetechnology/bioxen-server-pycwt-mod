...
signif, fft_theor = wavelet.significance(
    1.0, ds.dt, scales, 0, alpha, significance_level=slevel, wavelet=mother
)
...
glbl_signif, tmp = wavelet.significance(
    std2, ds.dt, scales, 1, alpha, significance_level=slevel, dof=dof, wavelet=mother
)
...
bx.contour(
    time, numpy.log2(period), sig95, [-99, 1], colors="k", linewidths=2, extent=extent
)
bx.fill(
    numpy.concatenate(
        [time, time[-1:] + ds.dt, time[-1:] + ds.dt, time[:1] - ds.dt, time[:1] - ds.dt]
    ),
    numpy.concatenate(
        [
            numpy.log2(coi),
...
bx.set_title("b) {} Wavelet Power Spectrum ({})".format(ds.label, mother.name))
...
cx.plot(
    std2 * fft_power, numpy.log2(1.0 / fftfreqs), "-", color="#cccccc", linewidth=1.0
)
